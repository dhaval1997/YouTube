    ...
    <div className="w-full my-2 flex max-sm:flex-col">
      <div
        className={`rounded-xl relative ${
          !imageLoaded &&
          `bg-gray-200 ${checkId ? "w-44 h-28" : "w-full sm:w-80 h-40"}`
        }`}
      >
        <img
          ...
          className={`rounded-xl hover:rounded-none ${
            checkId ? "w-44 h-28" : "w-full h-full"
          }`}
        />
        {isLive === "none" ? (
          <span
            className={`absolute ${
              checkId ? "bottom-6" : "bottom-2"
            } right-2 opacity-80 rounded-md pl-1 w-11 text-white bg-[#080808ec]`}
          >
            {_duration}
          </span>
        ) : (
          <span
            className={`absolute ${
              checkId ? "bottom-6" : "bottom-3"
            } right-2 opacity-90 rounded-md pl-2 w-11 text-white bg-red-600`}
          >
            Live
          </span>
        )}
      ...
      <div
        className={`flex flex-row m-0 sm:ml-4 max-sm:mt-2 ${
          checkId ? "w-52 h-32" : "w-full"
        }`}
      >
        <ul className="overflow-x-hidden pr-6">
          <li
            className={`${
              checkId ? "text-sm font-bold" : "text-xl font-normal"
            } mt-3 line-clamp-2 text-[#0f0f0f]`}
          >
            {title}
          </li>
          ...
          {!checkId && (
            <li className="hidden sm:block text-[#606060] text-sm line-clamp-2">
              {description}
            </li>
          )}
        </ul>
